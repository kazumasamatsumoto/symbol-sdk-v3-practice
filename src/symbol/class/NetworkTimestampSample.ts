...
async function runNetworkTimestampSample() {
    ...
    console.log(`現在のネットワークタイムスタンプ: ${currentTimestamp.toString()}`);
    
    // ネットワークタイムスタンプから日時に変換
    const convertedDatetime = myNetwork.toDatetime(currentTimestamp);
    console.log(`タイムスタンプから変換した日時: ${convertedDatetime.toISOString()}`);
    ...
    const combinedTimestamp = currentTimestamp
        ...
        .addMinutes(15)
        .addSeconds(30);
    console.log(`2時間15分30秒後のタイムスタンプ: ${combinedTimestamp.toString()}`);
    console.log(`2時間15分30秒後の日時: ${myNetwork.toDatetime(combinedTimestamp).toISOString()}`);
    ...
    // 非エポックタイムスタンプの確認
    console.log(`現在のタイムスタンプがエポックか: ${currentTimestamp.isEpochal}`);
    ...
    console.log(`MAINNET日時に変換: ${Network.MAINNET.toDatetime(mainnetNow).toISOString()}`);
    
    // TESTNETの使用
    const testnetNow = Network.TESTNET.fromDatetime(now);
    console.log(`TESTNET現在のタイムスタンプ: ${testnetNow.toString()}`);
    console.log(`TESTNET日時に変換: ${Network.TESTNET.toDatetime(testnetNow).toISOString()}`);
    ...
    console.log(`タイムスタンプ3 < タイムスタンプ4: ${timestamp3.timestamp < timestamp4.timestamp}`);
    console.log(`タイムスタンプ3 > タイムスタンプ4: ${timestamp3.timestamp > timestamp4.timestamp}`);
    console.log(`タイムスタンプ3 === タイムスタンプ4: ${timestamp3.timestamp === timestamp4.timestamp}`);
}
